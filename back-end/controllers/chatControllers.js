...
const accessChat = AsyncHandler(async (req,res) =>{
    ...
    .populate("lattestMessage");

    isChat = await User.populate(isChat,{
        path: "lattestMessage.sender",
...
const createGroupChat = AsyncHandler(async (req,res) =>{
    ...
    if(users.length<2){
        return res.status(400).send({error: "Please select at least two users."});
    }
...
const addToGroup = AsyncHandler(async (req,res)=>{
    ...
    const userExists = chat.users.includes(userId);
    if (userExists) {
        res.status(400);
